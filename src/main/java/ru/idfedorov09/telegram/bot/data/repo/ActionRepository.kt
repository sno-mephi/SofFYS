...
interface ActionRepository : JpaRepository<Action, Long> {
    ...
    @Query("SELECT COUNT(a) + 1 FROM actions_table a WHERE a.teamId = :teamId AND a.problemId = :problemId AND a.action = 'SEND_ANSWER'")
    ...
    @Query(
        "SELECT a.answer FROM actions_table a " +
